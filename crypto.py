...
def candidate_dict_generator(words, suffix):
    ...
    for counter in range(candidate_count):
        if counter < lw:
            # If counter is less than lw, yield a word without a suffix
            yield words[counter]
        else:
            # If counter is equal to or greater than lw, yield a word concatenated with a suffix
            word_index = (counter - lw) // lc
            suffix_index = (counter - lw) % lc
            yield words[word_index] + suffix[suffix_index]
